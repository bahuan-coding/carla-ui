...
import { Activity, AlertTriangle, ArrowRight, BadgeCheck, Clock4, Database, Link2, MoreHorizontal, RefreshCw, Repeat2, ShieldCheck, Smartphone, Sparkles } from 'lucide-react';
...
const toneBadge = (status?: string) => {
  const text = status?.toLowerCase?.() || '';
  if (/error|fail|reject|blocked/.test(text)) return 'bg-destructive/15 text-destructive';
  if (/retry|pending|queued|processing|bank/.test(text)) return 'bg-amber-500/15 text-amber-200';
  return 'bg-emerald-500/15 text-emerald-200';
};

const toneDot = (status?: string) => {
  const text = status?.toLowerCase?.() || '';
  if (/error|fail|reject|blocked/.test(text)) return 'bg-destructive';
  if (/retry|pending|queued|processing|bank/.test(text)) return 'bg-amber-300';
...
export function ProcesosPage() {
  ...
  const cards = processes.map((p) => ({
    id: p.id,
    title: p.name || p.phone || `Processo ${p.id}`,
    status: p.status || p.banking_status || '—',
    phone: p.phone,
    attempts: p.attempts,
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-border/50 bg-surface/80 px-4 py-3 shadow-[0_10px_50px_rgba(0,0,0,0.28)] backdrop-blur-xl">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-lg font-semibold text-foreground">Processos de WhatsApp · 360 Backoffice</h3>
          ...
          <p className="text-xs text-foreground/60">Fluxos de abertura/crédito com status, tentativas e timeline bancário/verificação.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Input
          placeholder="Buscar por phone, id, status"
          value={search}
      ...
      <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <Card className="glass border-border/60 bg-surface/90 text-foreground">
          <CardHeader className="flex flex-wrap items-center justify-between gap-2 pb-3">
            <CardTitle className="text-sm font-semibold">Processos em tempo real</CardTitle>
              ...
              : cards.map((card) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-1">
                        <p className="text-[11px] uppercase tracking-[0.12em] text-foreground/60">{card.id}</p>
                        <h4 className="text-sm font-semibold text-foreground">{card.title}</h4>
                      </div>
                      <span className={`rounded-full px-2 py-1 text-[11px] ${toneBadge(card.status)}`}>{card.status}</span>
                    </div>
                    <div className="space-y-1 text-xs text-foreground/60">
                      <div className="flex items-center gap-2">
                        <Smartphone size={12} /> {card.phone || '—'}
                      </div>
                      <div className="flex items-center gap-2">
                        <BadgeCheck size={12} /> Verif: {card.verification || '—'}
                      </div>
                      <div className="flex items-center gap-2">
                        <Database size={12} /> Banking: {card.banking || '—'}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-[11px] text-foreground/60">
                      ...
                      <span className="flex items-center gap-1 rounded-full bg-foreground/10 px-2 py-1">
                        <ShieldCheck size={12} /> Tentativas: {card.attempts ?? '—'}
                      </span>
                      <span className="flex items-center gap-1 rounded-full bg-foreground/10 px-2 py-1">
                    ...
                    {card.lastError ? (
                      ...
                      <div className="rounded-lg border border-border/40 bg-background/60 px-3 py-2 text-[11px] text-foreground/70">Fluxo operacional OK.</div>
                    )}
                    <div className="flex items-center justify-between gap-2 pt-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (!confirmDanger('Recolocar em retry?')) return;
                            retryMutation.mutate(
                              {},
                              { onError: (e) => onActionError('Falha ao retry', e), onSuccess: () => toast({ title: 'Retry enviado', description: 'Worker atualizado.' }) },
                            );
                          }}
                        >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!confirmDanger('Rerun inclui eventos. Continuar?')) return;
                          rerunMutation.mutate(
                            { include_events: true },
                            { onError: (e) => onActionError('Falha ao rerun', e), onSuccess: () => toast({ title: 'Rerun disparado', description: 'Worker em execução.' }) },
                          );
                        }}
                      >
        ...
        <Card className="glass border-border/60 bg-surface/90 text-foreground">
          <CardHeader className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold">
              <ArrowRight size={14} /> Quick actions
            </CardTitle>
            <Badge variant="outline" className="text-[11px] border-border/60 text-foreground/70">
              Auditoria + Dry run seguro
            </Badge>
          </CardHeader>
          <CardContent className="space-y-3 text-sm text-foreground/70">
            <div className="rounded-lg border border-border/50 bg-background/70 p-3">
              <p className="font-medium text-foreground">Forçar status</p>
              <p className="text-xs text-foreground/60">Use ao corrigir manualmente; pede motivo e operador.</p>
              <div className="mt-2 flex items-center gap-2">
                {['ready_for_bank', 'bank_processing', 'bank_retry', 'bank_rejected', 'account_created'].map((s) => (
                  <Button
                    ...
                    onClick={() => {
                      if (!confirmDanger(`Forçar status "${s}"?`)) return;
                      statusMutation.mutate(
                        { status: s },
                        { onError: (e) => onActionError('Falha ao forçar status', e), onSuccess: () => toast({ title: 'Status ajustado', description: s }) },
                      );
                    }}
                  >
                    {s.replace(/_/g, ' ')}
            ...
            <div className="rounded-lg border border-border/50 bg-background/70 p-3">
              ...
              <ul className="mt-1 space-y-1 text-xs text-foreground/65">
                <li className="flex items-center gap-2">
                  <BadgeCheck size={12} className="text-emerald-300" /> Use retry após corrigir payload.
                </li>
                <li className="flex items-center gap-2">
                  <AlertTriangle size={12} className="text-amber-300" /> Rerun é mais pesado: inclui eventos e worker completo.
                </li>
                <li className="flex items-center gap-2">
                  <Activity size={12} className="text-accent" /> Timeline mostra correlation_id e passos bancários.
                </li>
              </ul>
            </div>
          </CardContent>
        ...
        <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
          <SheetHeader>
            <SheetTitle className="flex items-center gap-2 text-sm">
              <ShieldCheck size={16} className="text-accent" /> Detalhe do processo
            </SheetTitle>
          </SheetHeader>
            ...
            <div className="space-y-3 py-4 text-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-base font-semibold text-foreground">{detailQuery.data.name || detailQuery.data.phone || 'Processo'}</p>
                </div>
                <span className={`rounded-full px-2 py-1 text-[11px] ${toneBadge(detailQuery.data.status || detailQuery.data.banking_status)}`}>
                  {detailQuery.data.status || detailQuery.data.banking_status || '—'}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[12px] text-foreground/70">
                <div className="rounded-lg border border-border/50 bg-background/70 p-2">
                  <p className="text-[11px] uppercase tracking-[0.12em] text-foreground/60">Phone</p>
                  <p>{detailQuery.data.phone || '—'}</p>
                </div>
                <div className="rounded-lg border border-border/50 bg-background/70 p-2">
                  <p className="text-[11px] uppercase tracking-[0.12em] text-foreground/60">Tentativas</p>
                  <p>{detailQuery.data.attempts ?? '—'}</p>
                </div>
                <div className="rounded-lg border border-border/50 bg-background/70 p-2">
                  <p className="text-[11px] uppercase tracking-[0.12em] text-foreground/60">Banco</p>
                  <p>{detailQuery.data.banking_status || '—'}</p>
                </div>
                <div className="rounded-lg border border-border/50 bg-background/70 p-2">
                  <p className="text-[11px] uppercase tracking-[0.12em] text-foreground/60">Verificação</p>
                  <p>{detailQuery.data.verification_status || '—'}</p>
                </div>
              </div>
